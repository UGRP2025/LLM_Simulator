...
from car_control.planner.speed_profiler import SpeedProfiler
...
class BehaviorPlanner(Node):
    def __init__(self, offline_mode=False, log_path=None):
        ...
        self.speed_profiler = SpeedProfiler(self.params['speed'])
        lane_paths = self.params['lane_csv_paths']
        self.lanes = load_three_lanes(lane_paths['center'], lane_paths['inner'], lane_paths['outer'])
        lanes_dict = {'center': self.lanes.center, 'inner': self.lanes.inner, 'outer': self.lanes.outer}
    ...
    def control_loop(self):
        ...
        if self.last_ips_time is None or (current_time - self.last_ips_time).nanoseconds / 1e9 > self.data_timeout_sec:
            self.get_logger().error(f"IPS data is stale... Halting control.")
        ...
        if self.last_imu_time is None or (current_time - self.last_imu_time).nanoseconds / 1e9 > self.data_timeout_sec:
            self.get_logger().error(f"IMU data is stale... Halting control.")
            self.throttle_pub.publish(Float32(data=0.0))
            self.steer_pub.publish(Float32(data=0.0))
            return
        ...
        context_data = {'ego': {'speed': self.current_speed, 'current_lane': self.current_lane}, 'risks': risks, 'metrics': metrics_by_lane}
        ...
        v_cmd = self.speed_profiler.target_speed(metrics_by_lane[self.current_lane], risks, hint)
        
        normalized_throttle = self.speed_profiler.speed_to_throttle(v_cmd)
        throttle_scale = self.params['speed'].get('throttle_scale', 1.0) 
        final_throttle = normalized_throttle * throttle_scale
        ...
        if not self.offline_mode:
            ...
            log_msg = f"Lane: {self.current_lane: <6} | Steer: {steer: .2f}, Thr: {final_throttle:.2f} | Risks: {risks_summary} | VLM: {vlm_summary}"
            self.get_logger().info(log_msg)
            self.steer_pub.publish(Float32(data=float(steer)))
            self.throttle_pub.publish(Float32(data=float(final_throttle)))
        else:
            return {'timestamp': time.time(), 'pose': self.pose, 'yaw': self.yaw, 'chosen_lane': self.current_lane, 'steer': steer, 'throttle': final_throttle, 'vlm_hint': hint, 'risks': risks}
